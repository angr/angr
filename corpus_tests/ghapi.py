...
class GitHubRepo:
    ...
    def get_default_token(self) -> str:
        try:
            ...
            raise RuntimeError("Failed to retrieve GitHub auth token using `gh auth token`") from e

    def get_branch_sha(self) -> str | None:
        url = f"https://api.github.com/repos/{self.owner}/{self.repo}/git/refs/heads/{self.branch}"
        logger.debug(f"Hitting URL: {url}")
        headers = {"Authorization": f"token {self.auth_token}", "Accept": "application/vnd.github.v3+json"}
    ...
    def push_file(self, local_file_path: str, repo_file_path: str, commit_message: str) -> bool:
        ...
        data = {
            ...
            "committer": {"name": self.user, "email": self.email},
    ...
    def create_pr(self, branch_to_merge: str, title: str) -> bool:
        ...
        logger.debug(f"Hitting URL: {url}")
        headers = {"Authorization": f"token {self.auth_token}", "Accept": "application/vnd.github.v3+json"}
        data = {"title": title, "head": f"{self.owner}:{branch_to_merge}", "base": self.branch}
...
def main():
    ...
    parser.add_argument(
        "--auth-token", "-A", type=str, default=None, help="GitHub authentication token [gh auth token]"
    )
    parser.add_argument(
        "--branch", "-b", type=str, default=DEFAULT_BRANCH, help=f'Source branch name ["{DEFAULT_BRANCH}"]'
    )
    ...
    _get_branch_sha_parser = subparsers.add_parser("get_branch_sha", help="Get the SHA of the specified branch")
    ...
    create_branch_parser.add_argument(
        "--allow-existing-branch", action="store_true", help="Allow the branch to exist without error"
    )
    ...
    push_file_parser.add_argument(
        "commit_message",
        type=str,
        nargs="?",
        default=DEFAULT_COMMIT_MESSAGE,
        help=f'Commit message ["{DEFAULT_COMMIT_MESSAGE}"]',
    )
