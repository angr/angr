...
def concretize_exit(current_exit, fallback_state):
	...
	try:
		l.debug("Constraints_after: %s" % current_exit.state.constraints_after())
...
def handle_exit(project, current_exit, fallback_state):
	...
	if concrete_start != None:
		l.debug("Concrete_start = 0x%x" % concrete_start)
		try:
			sirsb = handle_exit_concrete(project, concrete_start, current_exit)
		except SimIRSBError:
			l.warning("SimIRSB error caught. Skipping this one.", exc_info=True)
		except SimMemoryError:
			l.warning("Constraints are not satisfiable. This exit should not be taken.")
	else:
