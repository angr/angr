...
class DDG(object):
    ...
    def construct(self):
        ...
        initial_container = AddrToRefContainer(
            self._cfg.get_irsb(self._entry_point),
            {})
        ...
        while len(run_stack) > 0:
            ...
            if isinstance(run, SimIRSB):
                ...
                for i in range(len(statements)):
                    ...
                    if len(refs) > 0:
                        ...
                        if isinstance(real_ref, SimMemWrite):
                            addr = real_ref.addr
                            if not addr.is_symbolic():
                                concrete_addr = addr.any()
                                container.addr_to_ref[
                                    concrete_addr] = (irsb, i)
                            else:
            ...
            for successor in successors:
                new_container = AddrToRefContainer(
                    successor,
                    container.addr_to_ref.copy())
