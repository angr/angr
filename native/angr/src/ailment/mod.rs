...
///
/// `u64::MAX` is what angr already stores for an address it does not have one
/// layer up: `CFGModel.serialize_to_cmessage` writes it for a CFG edge with no
/// `ins_addr`.
pub const INVALID_ADDR: Addr = Addr::MAX;

/// Serde shim putting an [`Addr`] on the wire in the signed form the AIL
/// address fields carried through angr v9.3.4.
///
