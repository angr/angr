    ...
    pub fn urem(&self, other: &Self) -> Result<Self, ClarirsError> {
        match (self, other) {
            ...
            (
                ...
                // Simple case: both are constants. The dividend has to be one
                // too: `s_lb == o_lb` only says its lower bound happens to equal
                // the divisor, and `[c, u] urem c` is not the constant `c % c`.
                if self.is_integer() && other.is_integer() {
