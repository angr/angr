...
class SimIRStmt(object):
    ...
    __slots__ = [ 'stmt', 'imark', 'stmt_idx', 'state', 'options', 'refs', 'exit_taken' ]
    ...
    def _record_expr(self, expr):
        ...
        self._add_constraints(*expr.constraints)
    ...
    def _translate_exprs(self, exprs):
        ...
        for expr in exprs:
            expr_ = self._translate_expr(expr, constraints)
            s_exprs.append(expr_)
            constraints += expr_.constraints

        return s_exprs, constraints

    # translates an IRExpr into a SimIRExpr, honoring the state, mode, and
    # options of this statement
    def _translate_expr(self, expr, extra_constraints=None):
    ...
    def _write_tmp(self, tmp, expr):
        ...
        # However, if we're in concrete mode, we store the temps in the list
        # directly.
    ...
    def _handle_CAS(self, stmt):
        ...
        self._record_expr(expd_lo)
        if double_element:
            expd_hi = self._translate_expr(stmt.expdHi)
            self._record_expr(expd_hi)
        ...
        addr_first = self.state.expr_value(
            symexec.BitVecVal(addr, self.state.arch.bits))
        addr_second = self.state.expr_value(
            symexec.BitVecVal(addr + element_size, self.state.arch.bits))
        ...
        # load lo
        old_lo = self.state.mem_expr(
            addr_lo, element_size, endness=stmt.endness)
        ...
        old_hi = None
        if double_element:
            old_hi = self.state.mem_expr(
                addr_hi, element_size, endness=stmt.endness)
    ...
    def _handle_Dirty(self, stmt):
        ...
        if hasattr(s_dirty, stmt.cee.name):
            ...
            # get the size, and record the write
            if o.TMP_REFS in self.options:
                self.refs.append(SimTmpWrite(
                    self.imark.addr, self.stmt_idx, stmt.tmp, sim_value, retval.size() / 8, [], []))

            self._write_tmp(stmt.tmp, sim_value.expr)
        else:
    ...
    def _handle_LoadG(self, stmt):
        ...
        read_expr = self.state.mem_expr(
            addr.expr, read_size, endness=stmt.end)
        if read_size == converted_size:
            converted_expr = read_expr
        elif "S" in stmt.cvt:
            converted_expr = symexec.SignExt(
                converted_size - read_size, read_expr)
        elif "U" in stmt.cvt:
            converted_expr = symexec.ZeroExt(
                converted_size - read_size, read_expr)
        else:
            raise Exception("Unrecognized IRLoadGOp %s!", stmt.cvt)

        expr = symexec.If(guard.expr != 0, converted_expr, alt.expr)
        self._write_tmp(stmt.dst, expr)
    ...
    def _handle_StoreG(self, stmt):
        ...
        old_data = self.state.mem_expr(
            addr.expr, write_size, endness=stmt.end)
        write_expr = symexec.If(guard.expr != 0, data.expr, old_data)
        old_data = self.state.store_mem(
            addr.expr, write_expr, endness=stmt.end)
