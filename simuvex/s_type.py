...
class SimTypeReg(SimType):
	...
	def is_sub(self, other):
		return super(SimTypeReg, self).is_sub(other) and self.size == other.size
...
class SimTypePointer(SimTypeReg):
	...
	def __init__(self, pts_to, label=None):
		...
		'''
		SimTypeReg.__init__(self, 64, label=label) # TODO: ffffuuuuuuuu fix pointer size
		self.pts_to = pts_to
...
class SimTypeLength(SimTypeInt):
	...
	def __init__(self, addr=None, length=None, label=None):
		...
		'''
		SimTypeInt.__init__(self, 64, False, label=label) # again assuming 64-bit
		self.addr = addr
		self.length = length
