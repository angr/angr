...
class SimArch:
	def __init__(self, bits, vex_arch, name, max_inst_bytes, ip_offset, sp_offset, bp_offset, ret_offset, stack_change, memory_endness, register_endness, ret_instruction, nop_instruction):
		self.bits = bits
...
class SimAMD64(SimArch):
	def __init__(self):
		SimArch.__init__(self, 64, "VexArchAMD64", "AMD64", 15, 184, 48, 56, 16, -8, "Iend_LE", "Iend_LE", "\xc3", "\x90")

class SimX86(SimArch):
	def __init__(self):
		SimArch.__init__(self, 32, "VexArchX86", "X86", 15, 68, 24, 28, 8, -4, "Iend_LE", "Iend_LE", "\xc3", "\x90")
...
class SimARM(SimArch):
	def __init__(self):
		...
		SimArch.__init__(self, 32, "VexArchARM", "ARM", 4, 68, 60, 60, 8, -4, "Iend_LE", "Iend_LE", "\x0E\xF0\xA0\xE1", "\x00\x00\x00\x00")

class SimMIPS32(SimArch):
	def __init__(self):
		# TODO: multiple return registers?
		# TODO: which endianness?
		SimArch.__init__(self, 32, "VexArchMIPS32", "MIPS32", 4, 128, 116, 120, 8, -4, "Iend_LE", "Iend_LE", "\x08\x00\xE0\x03" + "\x25\x08\x20\x00", "\x00\x00\x00\x00")
...
class SimPPC32(SimArch):
	def __init__(self):
		...
		SimArch.__init__(self, 32, "VexArchPPC32", "PPC32", 4, 1168, 20, -1, 8, -4, "Iend_BE", "Iend_BE", "\x4e\x80\x00\x20", "\x60\x00\x00\x00") # 4e800020: blr
