...
class b64_decode(simuvex.SimProcedure):
	def __init__(self): # pylint: disable=W0231
		...
		self.state.store_mem(dst+16, self.state.new_bvv(0, 8))
...
#		self.state.store_mem(one+4, self.state.new_bvv(0, 8))
#		self.inline_call(memcpy, two, src+6, 8192)
#		self.state.store_mem(two+8191, self.state.new_bvv(0, 8))
