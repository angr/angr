...
class __tls_get_addr(simuvex.SimProcedure):
    def run(self, index):
        mod_id = self.state.mem_expr(index, self.state.arch.bits / 8, endness=self.state.arch.memory_endness)
        ...
        offset = self.state.mem_expr(index + 4, self.state.arch.bits / 8, endness=self.state.arch.memory_endness)
