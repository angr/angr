...
class mempcpy(angr.SimProcedure):
    ...
    def run(self, dst_addr, src_addr, limit):
        if not self.state.solver.symbolic(limit):
            ...
            if max_limit > max_memcpy_size and conditional_size < max_limit:
                l.warning(
                    "mempcpy upper bound of %#x outside limit, limiting to %#x instead", max_limit, conditional_size
                )

        l.debug("mempcpy running with conditional_size %#x", conditional_size)
