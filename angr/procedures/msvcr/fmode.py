...
class __p__fmode(angr.SimProcedure):
    def run(self):
        return self.project._sim_environment.fmode_ptr

class _get_fmode(angr.SimProcedure):
    def run(self, outptr):
        if self.state.solver.is_true(outptr == 0):
            return 22
        fmode = self.state.mem[self.project._sim_environment.fmode_ptr].int.resolved
...
class _set_fmode(angr.SimProcedure):
    def run(self, val):
        if not self.state.solver.is_true(self.state.solver.Or(val == 0x4000, val == 0x8000)):
            return 22
        self.state.mem[self.project._sim_environment.fmode_ptr].int = val
        return 0

class __p__commode(angr.SimProcedure):
    def run(self):
        return self.project._sim_environment.commode_ptr
