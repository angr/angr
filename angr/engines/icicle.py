...
class IcicleEngine(SuccessorsEngine):
    ...
    @staticmethod
    def _make_icicle_arch(arch: Arch) -> str | None:
        """
    ...
    @staticmethod
    def _is_arm(icicle_arch: str) -> bool:
        """
    ...
    @staticmethod
    def _is_cortex_m(angr_arch: Arch, icicle_arch: str) -> bool:
        """
    ...
    @staticmethod
    def _is_thumb(angr_arch: Arch, icicle_arch: str, addr: int) -> bool:
        """
        Check if the architecture is thumb based on the address.
        """
        return IcicleEngine._is_cortex_m(angr_arch, icicle_arch) or (
            IcicleEngine._is_arm(icicle_arch) and addr & 1 == 1
        )

    @staticmethod
    def _syscall_jumpkind(arch_name: str, emu: Icicle) -> str:
    ...
    @staticmethod
    def _get_explicit_page_metadata(state: SimState[int, int]) -> dict[int, int | None]:
        """
    ...
    @staticmethod
    def _sync_registers(emu: Icicle, state: SimState[int, int], register_names: Iterable[str]) -> set[str]:
    ...
    @staticmethod
    def _write_page(emu: Icicle, state: SimState[int, int], page_num: int) -> None:
    ...
    @staticmethod
    def _sync_edge_hitmap(emu: Icicle, state: SimState[int, int]) -> None:
    ...
    @staticmethod
    def _build_emu_for(state: SimState[int, int]) -> tuple[Icicle, IcicleStateTranslationData]:
    ...
    def _convert_icicle_state_to_angr(
        ...
        if IcicleEngine._is_arm(emu.architecture):  # Hack to work around us calling it r15t
    ...
    def _sync_state_to_emu(
        ...
        page_size = state.memory.page_size
        explicit_page_metadata = IcicleEngine._get_explicit_page_metadata(state)
        if base is None:
    ...
    def _run_icicle(
        ...
        added_breakpoints = []
        is_arm = IcicleEngine._is_arm(translation_data.icicle_arch)
