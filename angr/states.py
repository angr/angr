...
class StateGenerator(object):
    ...
    def entry_point(self, mode, args=None, env=None, sargc=None, **kwargs):
        ...
        @param args - Argv for the program as a python list. Optional, defaults
                      to ['./' + binary name].
        @param env - The program's environment as a python dict. Optional, defaults to empty.
        @param sargc - If true, argc will be fully unconstrained. Be very afraid.
        ...
        # Handle default values for args/env
        if args is None:
            args = ['./' + self._ld.main_bin.binary]
        if env is None:
            env = {}
        ...
        if self._ld.main_bin.ppc64_initial_rtoc is not None:
            state.store_reg('rtoc', self._ld.main_binary.ppc64_initial_rtoc)
            state.abiv = 'ppc64_1'
