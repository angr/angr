...
class SimEngineVRAIL(
    ...
    def _call_add_arg_based_type_constraints(
        ...
        for arg, arg_atom, arg_type in zip(args, arg_atoms, prototype.args):
            ...
            if arg.typevar is not None and isinstance(
                ...
                if stack_offset is not None and isinstance(arg_ty, typeconsts.Pointer):
                    stack_var = self.state.variable_manager[self.func_addr].find_variable_by_atom(
                        self.block.addr, self.stmt_idx, arg_atom, block_idx=self.block.idx
                    )
                    if stack_var is not None:
                        # there is an existing stack variable for this atom, and it means we have already visited
                        # this atom and added type constraints. skip.
                        continue

                    if isinstance(arg_ty.basetype, typeconsts.Struct):
