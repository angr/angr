...
class CFGFast(ForwardAnalysis[CFGNode, CFGNode, CFGJob, int, object], CFGBase):  # pylint: disable=abstract-method
    ...
    def _scan_for_nop_padding(self, start_addr: int) -> int:
        ...
        inside, scan_end = self._inside_regions_and_region_end(start_addr)
        if not inside or scan_end is None:
            return 0
        next_occupied = self._seg_list.next_pos_with_sort_not_in(start_addr, set())
        if next_occupied is not None:
            scan_end = min(scan_end, next_occupied)

        addr = start_addr
        while addr < scan_end:
            nop_size = nop_size_at(addr)
            if not nop_size or addr + nop_size > scan_end:
                break
            addr += nop_size
    ...
    def _capture_resume_job(job: CFGJob) -> ResumeJob:
        ...
        # only integer source addresses can be re-resolved against the model when resuming
        ...
        return ResumeJob(
            ...
            src_node_addr=src_node_addr if isinstance(src_node_addr, int) else None,
