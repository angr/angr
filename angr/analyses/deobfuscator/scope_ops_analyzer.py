...
class ScopeOpsWalker(CStructuredCodeWalker):
	...
	def handle(self, obj):
		if type(obj).__name__ in [ "CFunction", "CWhileLoop", "CDoWhileLoop", "CSwitchCase", "CIfBreak", "CForLoop", "CIfElse" ]:
...
class ScopeOpsAnalyzer(Analysis):
	...
	def filter_scopes(self, wanted_ops: set, min_count):
		return [ a for a,s in self.scope_ops.items() if sum(self.scope_ops[a][v] for v in s if v in wanted_ops) >= min_count ]
