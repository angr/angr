...
class CodeLocation:
    """
    Stands for a specific program point by specifying basic block address, block ID and statement ID (for IRSBs), or SimProcedure
        ...
        if self.block_addr is None:
            ...
            if self.stmt_idx is None:
                s = "<%s%#x(-)%s" % (
                    ...
                    self.block_addr, (self.block_id) if self.block_id else ""
                )
            else:
                s = "<%s%#x[%d]%s" % (
                    ("%#x id=" % self.ins_addr) if self.ins_addr else "",
                    self.block_addr,
                    self.stmt_idx, (self.block_id) if self.block_id else ""
                )
    ...
    def __hash__(self):
        ...
        """
        return hash((self.block_addr, self.stmt_idx, self.sim_procedure, self.block_id))
