            ...
            if cc.RETURN_VAL is not None:
                if isinstance(cc.RETURN_VAL, SimRegArg):
                    ...
                    if func_addr is not None and len(func_addr) == 1 and self.functions is not None and type(func_addr_int) != Undefined:
                        self.state.kill_and_add_definition(atom, self._codeloc(), DataSet({undefined}, reg_size * 8), tag=Tag(name="retval", metadata=hex(func_addr_int)))
                    else:
                        self.state.kill_and_add_definition(atom, self._codeloc(), DataSet({undefined}, reg_size * 8), tag=Tag(name="retval", metadata=None))
