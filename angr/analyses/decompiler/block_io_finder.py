...
from angr.ailment.utils import is_llsc_expression, is_store_conditional_expression
...
class BlockIOFinder(AILBlockViewer):
    ...
    def can_swap(self, stmt, ail_obj: Block | list[Statement], offset: int):
        ...
        if (
            ...
            or
            # LL/SC operations are atomic side effects and cannot be reordered.
    ...
    def _handle_stmt(self, stmt_idx: int, stmt: Statement, block: Block | None):
        if block is None:
            if (
                isinstance(stmt, DirtyStatement)
                and self._current_top_stmt_idx is not None
                and is_llsc_expression(stmt.dirty)
            ):
    ...
    def _handle_DirtyExpression(
        ...
        if is_llsc_expression(expr) and self._current_top_stmt_idx is not None:
            self.side_effects_at.add(self._current_top_stmt_idx)
        ...
        if expr.maddr is not None:
            ...
            if memory_loc is not None:
                ...
                if is_store_conditional_expression(expr):
                    self._add_or_update_dict(self.outputs_by_stmt, stmt_idx, memory_loc)
                elif is_llsc_expression(expr):
