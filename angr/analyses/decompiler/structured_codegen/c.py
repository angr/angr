...
class CIndexedVariable(CExpression):
    ...
    def c_repr_chunks(self, indent=0, asexpr=False):
        ...
        if _is_pointer_to_array_type(variable.type):
            paren = CClosingObject("(")
            yield "(", paren
            yield "*", self
...
class CStructuredCodeGenerator(BaseStructuredCodeGenerator, Analysis):
    ...
    def _analyze(self):
        ...
        self._use_stack_backing = False
        self._stack_backing_base_offset = None
        self._stack_backing_size = 0
    ...
    def _handle_Stmt_Assignment(self, stmt, **kwargs):
        if (
            ...
            csrc = _scalarize_array_operand(self, csrc, cdst.type)
...
class SignedComparisonVariableFixer(CStructuredCodeWalker):
    ...
    def handle_CFunction(self, obj):
        ...
        for var, cset in obj.unified_local_vars.items():
            if var not in self.collector.signed_vars:
                new_locals[var] = cset
                continue
            new_cset = set()
            for cvar, ty in cset:
