...
(scales with the architecture). Where a runtime differs in a less regular way, the conditional lives here:
the MSVC std::string keeps its size behind a fixed 16-byte SSO buffer union
while libstdc++ keeps it one word past the data pointer.
"""
