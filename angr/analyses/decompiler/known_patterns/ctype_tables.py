...
``<ctype.h>`` classifies a character by indexing a locale table rather than by
comparing ranges::
...
macro-oracle sites across 24 projects, wider project coverage than any family
KnownPatterns covers today, with ``tolower`` and ``toupper`` adding 1,588 more.
...
the low byte. gcc tests the high ones with ``test $0x20,%ah``, an 8-bit slice at
byte offset 1, rather than masking the full 16-bit word, so each predicate needs
both spellings.
...
def _indexed_load(names: frozenset[str], elem_size: int) -> PDefOf:
    ...
    mask is applied to a register rather than to a Load.
    """
...
def _build_predicate(macro: str, mask: int):
    def build(ctx: PatternContext) -> KnownPattern:
        ...
        if mask >= 0x100:
            ...
            # mask's high byte. Not a Convert, since a Convert takes the low end.
