...
looks like the way to tell them apart. It selects the wrong cases. The folded
...
which is the case the pattern gets right, does not, since the pointer is handed
to a callee or stored. Measured on putty/gcc it dropped function-level recall
from 90.7% to 65.0% and site-level recall from 42.7% to 2.1%, and it also
rejects the ``sum_list`` fixture below. The offset floor already removes the
decrements it was aimed at.
"""
