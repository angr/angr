"""The CONTAINING_RECORD macro, common in Windows driver code:
...
This shape is too generic to run by default (any pointer arithmetic matches),
so the pattern is not enabled unless it is passed explicitly via
...
def _record_returnty(arch: archinfo.Arch, const_args: dict[str, int]) -> SimType | None:
    ...
    Subtype instead of an Equivalence so that the struct can grow)."""
