...
(commutative operand orders, :class:`PChoice`) can backtrack by reusing the
pre-branch state.
"""
...
class MatchCtx:
    ...
    # expression that can be recomputed wherever the vvar is live, such as a
    # CSE'd definition in a dominating block, possibly behind phis. Returns None
    # when the definition is not recomputable.
    ...
    # copies and through phis whose arms agree. This is not ``peek_fn``: that
    # one promises "this expression can be recomputed at the use", so it refuses
    # loads and calls. This one only answers "what produced this value", which a
...
class PExtract(PatternExpr):
    ...
    came from ``maxsd``/``mulsd`` lifts to ``Extract(vvar_128, 64bits@0)``
    rather than to a Convert. Without a node for it, every libm bit-twiddle
    whose operand is a computed double rather than an incoming argument is
    unmatchable.
...
class PCall(PatternExpr):
    ...
    std::string destructor, ``__ctype_b_loc`` in ``isspace``) cannot be confused
    with arithmetic that happens to look alike, because no other code calls
    that function to do something else.
...
class PCallResult(PatternExpr):
    ...
    A call's result is rarely used where it is produced: the compiler assigns
    it to a register and reads that register, often in another block.
    ...
    stays where the compiler put it.
    ...
    def match(self, expr: Expression, state: MatchState, ctx: MatchCtx) -> MatchState | None:
        ...
        if not isinstance(call, Call) or not self._target_matches(call, ctx):
            return None
        # bind the variable rather than the call: it is the value that exists at
        # the match site, and the only spelling the outliner could pass along
        return self._bind_if_named(self.name, expr, state)
...
class PDefOf(PatternExpr):
    ...
    through this node keeps it as residue. That is harmless, and dead-code
    elimination removes it once the idiom's other uses are gone.
    """
    ...
    def match(self, expr: Expression, state: MatchState, ctx: MatchCtx) -> MatchState | None:
        # The read-only lookup goes first, on purpose. The ordinary structural
        # match would reach the same definition through ``chase_fn``, but chasing
        # consumes the defining statement (it commits the outliner to moving
        # it), and a value the compiler hoisted is hoisted precisely because
        # something else still reads it. The chased match is then thrown away
...
class PField(PatternExpr):
    ...
    def match(self, expr: Expression, state: MatchState, ctx: MatchCtx) -> MatchState | None:
        ...
        # replace it by its definition, turning `Load(v)` into
...
class PStackField(PatternExpr):
    ...
    compare against. Measured over the benchmark corpus
    (`kp-eval/stack_share.py`), stack objects are 40-54% of the DWARF sites for
    ``std::string::length`` and 15-35% for ``std::vector<T>::size``.
    ...
    def match(self, expr: Expression, state: MatchState, ctx: MatchCtx) -> MatchState | None:
        ...
        if stack_off is None:
            return None
        # the base is an address, so it has pointer width rather than the slot's width, which is whatever variable
...
class PITE(PatternExpr):
    ...
    outliner as an expression, not as a PGraphPat region. (A triangle, with one
...
class PCallStmt(PatternStmt):
    ...
    both match; ``dst``, when given, also constrains the assigned destination,
    which requires the assignment form.
    """
...
class PStmtSeq(PatternStmt):
    ...
    (each pattern to a distinct statement), which suits idioms whose statement
    ...
    #: start at, quadratic per block per template, and finds nothing: an idiom's
    #: statements are adjacent apart from what the scheduler interleaved, which
    #: is a handful of instructions rather than a basic block.
    max_gap: int = 8
...
class PGraphPat(PatternNode):
    ...
    in ``blocks`` denotes an external successor, a region exit that becomes
    the Outliner frontier. ``entry`` is the label of the single entry block.
    Captures unify across all blocks (one bindings environment).
    """
...
def stmt_pattern_anchor_key(node: PatternNode) -> StmtKey | None:
    ...
    which is what separates ``t = *a`` (a swap) from the assignments that make
    up most of a block.
    """
    if isinstance(node, PStmtSeq):
