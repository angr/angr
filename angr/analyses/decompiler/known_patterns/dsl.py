...
class MatchCtx:
    ...
    # through phis whose arms agree. This is not ``peek_fn``: that one promises
    # "this expression can be recomputed at the use", which a call can never
    # satisfy. A call is only ever referred to.
...
class PStackField(PatternExpr):
    ...
    live-ins are N values rather than one pointer, and there is nothing for the
    ...
    but is a transformation rather than a capture.
    """
