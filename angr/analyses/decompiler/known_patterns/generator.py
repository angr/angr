...
- an expression selection (a sub-expression of one statement) becomes a
  :class:`~.dsl.PatternExpr` tree;
- a straight-line statement selection (statements of one basic block)
  becomes a :class:`~.dsl.PStmtSeq`;
- a selection spanning several basic blocks becomes a
  :class:`~.dsl.PGraphPat` (requires the finder's ``ail_graph`` for the block/edge structure).
