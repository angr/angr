...
``hlist_add_head`` is absent on purpose: its last two stores land in the
...
# POISON_POINTER_DELTA is CONFIG_ILLEGAL_POINTER_VALUE, which is
# 0xdead000000000000 on x86-64 and arm64. Requiring a non-zero high half keeps the fingerprint strong
...
# Opt-in everywhere, but gated on this binary being a kernel object. `list_head` and `LIST_ENTRY` are what
