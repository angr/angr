...
the callee being ``operator delete``. That is what makes this pattern
...
def _build_dtor(ctx: PatternContext, sized: bool) -> KnownPattern:
    ws = ctx.word_size
    # _M_p is read twice, once by the compare and once as the pointer to free,
...
def _build_dtor_stack(ctx: PatternContext, sized: bool) -> KnownPattern:
    ...
    compared against the address of the slot two words along and then freed,
    which is what :class:`~.dsl.PStackField` matches. This is the spelling a
    local ``std::string`` takes, and locals are what most destructors destroy.
    """
