...
* target evidence (:class:`TargetGate`): a fact about the binary as a whole,
  derived once into the :class:`~.context.PatternContext`: this is a Linux
  kernel module, this is a Windows kernel-mode driver. Cheap, exact, and
  evaluated before any matching happens.
* corroboration (:class:`CorroboratedBy`): another, unambiguous pattern already
  matched in the same function. This is what converts "too generic to ship"
  into "safe here": ``Load(Load(s))`` is meaningless on its own, but in a
