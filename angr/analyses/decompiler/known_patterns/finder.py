...
class KnownPatternFinder(Analysis):
    ...
    def _index_patterns(self) -> None:
        ...
        block would be quadratic in the size of the library. Statement patterns are
    ...
    def _match_all(self) -> list[KnownPatternMatch]:
        ...
        # load of every std::vector<T>::size). Among matches of the same size, the
        # one closest to the statement root (shortest expr_path) wins.
        ...
        # moved to where the call goes, which any side effect in between forbids,
        # so a spelling that describes the code as it stands is easier to apply.
        # Two statement orders of std::swap can cover one field swap: one reads
        # the second value inline and chases its definition, the other has both
        # loads hoisted, and only the second can be applied when the neighbouring
        # field's store sits between them. Graph and expression matches are left
        # out on purpose: their footprint is only their entry statement, so a
        # chase count would outrank match size and let a destructor's own guard
        # beat the destructor.
    ...
    def _try_match_graph(self, pattern: KnownPattern, entry_block: Block) -> KnownPatternMatch | None:
        ...
        # move a definition into. Peeking is still fine, since it reads a
        # definition without disturbing it, which is what lets a region whose
        # object address was computed into its own register (MSVC c_str, clang's
        # capacity triangle) match at all.
        ctx = MatchCtx(
            skip_conversions=self._skip_conversions,
            chase_fn=None,
            peek_fn=self._resolve_remote_def,
    ...
    def _try_match(
        ...
        # idiom are routinely reused from other blocks (that is why the compiler
        # kept them in registers), so rejecting the match outright makes a
        # pattern unreachable on the very code it targets.
        #
        ...
        if shared:
            ...
            # (_materialize_exprs); a call or a side effect is not.
            if not all(
    ...
    def _resolve_def(self, varid: int, _depth: int = 0, _seen: frozenset[int] | None = None) -> Expression | None:
        ...
        because re-evaluating them is not the same as reading their result. This
        one answers "what produced this value", which a load or a call may well
        have done, and the answer is only ever used to identify an idiom.
    ...
    def _resolve_remote_def(self, varid: int, _depth: int = 0, _seen: frozenset[int] | None = None):
        ...
        value. "Pure" here means register arithmetic only, with no load, no call
        and no dirty helper, so that no intervening store can change the result.
    ...
    def outline(self, match: KnownPatternMatch, ail_graph: networkx.DiGraph | None = None) -> OutlineResult:
        ...
        # A stack container is named in place rather than outlined. Its region's
        # inputs are N independent stack slots rather than one pointer, so there
        # is nothing for a synthesized callee to take, and nothing needs to be:
        ...
        if match.block_map is not None:
            try:
                ...
                if match.pattern.returnty is not None or match.pattern.returnty_factory is not None:
                    raise
                # A void region needs no callee, since nothing reads a value out
                # of it. So when outlining declines (v1 outlining wants exactly
                # one region exit, and a destructor whose arms both end the
        ...
        if match.stmt_span is not None:
            ...
            if void:
                ...
                # interface for an outlined callee to discover. Outlining would
                # also split the block, which is destructive here in a way it is
                # not for an expression: a block holding several interleaved
        ...
        try:
            ...
            # Outlining exists to discover a region's interface. When it cannot,
            # the pattern is not necessarily unusable: every argument the
            # pattern declared is an expression the matched statement already
            # had in hand, so the call can simply take the matched expression's
            # place. That is what rescues an idiom reading a value the compiler
            # hoisted out of it: the hoisted definition stays where it is, so
    ...
    #     *b  = t           <- matched
    #
    # so require disjointness rather than absence. Only the analyzable shape
    # counts: an address that is a variable plus a constant, against another
    # address on the same variable. Anything else is still refused.
    ...
    def _materialize_bases(
        ...
        is computed once *ahead* of the region, as ``tmp = root + K``, and the
    ...
    def _is_liftable(cls, expr: Expression) -> bool:
        ...
        the region: no calls, no side effects. A ``Load`` qualifies, since lifting
        it does not move it: the temporary lands directly in front of the
        statement the matched expression came out of, with nothing in between.

        Bounded in size. Rewriting the region to use the temporary has to find
        the occurrences, which means comparing the captured expression against
        every sub-expression of the statement with ``.likes()``: the product of
        two sizes, on statements that in a large optimized function are enormous.
        The captures this exists for are a field read or a variable behind a
        Convert; a whole computed sub-tree is neither cheap to find nor readable
        as an argument.
        """
        n = 0
        for sub in _iter_subexprs(expr):
    ...
    def _map_uses(obj, fn):
        ...
        the destination out of the walk by identity does not work, because the
        AIL objects are Rust-backed, so attribute access hands back a fresh Python
        wrapper each time and `is` never matches. Hence the statement is rebuilt
        around a rewritten source instead."""
    ...
    def _stack_ref(self, sbo: StackBaseOffset, blocks: Iterable[Block]) -> Expression:
        ...
        construction, since the pattern read it to bind the base, so we can find
        it rather than synthesize one.
        """
        for blk in blocks:
    ...
    def _check_gap_statements(self, stmts: list, span: list[int], consumed) -> list[int]:
        ...
        statements: outlining hoists those before the region, and rewriting in
        place leaves them after the call. Both are sound under the same
    ...
    def _prepare_stmt_rewrite(self, match: KnownPatternMatch, block: Block) -> tuple[list[int], int, list[Statement]]:
        ...
        rewrites. Re-matching the pattern is what makes a stale match safe:
        statement indices shift under every earlier rewrite, and a span applied
        blind would delete whatever now sits at those positions.
        """
        if match.pattern.returnty is not None or match.pattern.returnty_factory is not None:
            raise UnsupportedOutlineError(
        ...
        # outlining hoist them before the region allows that
        self._check_gap_statements(stmts, span, ())
        # A value the span defines and something outside still reads cannot just
        # be deleted. Outlining catches that through the recovered interface (a
        # void pattern with frontier variables is refused); this path has no
        # interface, so it asks directly. It need not give up, though: the
        # statement that produces such a value is a pure load, and keeping it in
        # front of the call preserves it exactly, because it read memory before
        # the idiom wrote any. Only what follows the last kept statement is
        # replaced. A swap whose temporary happens to be the function's
        # (over-approximated) return value needs this.
        kept = [
