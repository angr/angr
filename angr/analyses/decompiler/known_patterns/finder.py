...
class KnownPatternFinder(Analysis):
    ...
    def _resolve_call_def(self, varid: int, _depth: int = 0, _seen: frozenset[int] | None = None) -> Call | None:
        ...
        Read-only, and kept separate from :meth:`_resolve_remote_def`: that one
        answers "what can I recompute here" and refuses calls, because
        re-evaluating a call is not the same as reading its result. This one
        answers "what produced this value", which a call may well have done.
