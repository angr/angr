...
* the same, storing in the other order::
  ...
  which differs from the previous one with ``a`` and ``b`` renamed: that
  renaming also swaps the two loads, so the sequence differs either way round.
  gcc emits it for consecutive field swaps, where it lets the store to one
  field sit next to the load of the next.
...
def _held(cap: str) -> PChoice:
    ...
    than ``t``. The load side needs no help, since structural matching already
    skips Convert wrappers, but a bare PVVar does not, deliberately, so the store
