...
* as scheduled, with both loads hoisted, which is what any instruction
  scheduler produces, and what gcc emits for a struct assignment that swaps
  field by field::

      t1 = *a;  t2 = *b;  *a = t2;  *b = t1;

They cannot share a pattern object, since a statement sequence is one object, so
...
#:
#: Only the word size, and that is a performance decision rather than a coverage
#: one. A statement-sequence pattern is tried at every statement of every block
#: and then scans forward, so each extra template multiplies a quadratic term,
#: and the narrow widths are the ones that match most, since a three-statement
#: byte shuffle describes a great deal of code that is not a swap. With all four
...
#: patterns are already pruned by anchor key.
_SWAP_WIDTHS = (8,)
