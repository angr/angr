...
The shift amount is an 8-bit constant. One pattern is registered per
common element size (short/int/long long); ``sizeof(T) == 1`` (vector<char>)
is not covered, because without the shift the shape is a bare pointer
difference, which is too generic.
