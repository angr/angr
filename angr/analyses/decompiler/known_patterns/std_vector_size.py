...
definition covers 32- and 64-bit. ``sizeof(T) == 1`` is not covered (no shift, so a
bare pointer difference, which is too generic).
...
#
# that is, a right shift by the number of trailing zeros of sizeof(T) followed
# by a plain low-half multiply by the modular inverse of its odd part. Both
# operands are fully determined by sizeof(T), so one template per element size
# can be generated. Verified on g++ 12.2.0 at -O1/-Os/-O2/-O3 for sizeof(T) in
# {3,6,12,20,24,40,48,56,72,80}.
#
# Without these, such a vector is misidentified: the inner `diff >> 2` alone
...
# list is wide rather than a handful of common sizes: sizeof(T) is whatever the
# program's record happens to be (112 for CryptoPP::ECPPoint, say), and a size
# that is not covered leaves the raw magic multiply in the output, which is the
# shape this pattern exists to remove.
