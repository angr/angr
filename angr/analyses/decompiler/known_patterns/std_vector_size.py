...
# from). capacity() is generated over the same element sizes as size(): it had
# only the int template, and the benchmark corpus contains no 4-byte-element
# capacity sites at all, so it could not fire.
...
def _make_vector_div_template(
    ...
    division, since the compiler knows the byte difference is a whole multiple of
    ...
    Without them such a vector is misidentified: the inner ``diff >> 2`` alone
    ...
    def build(ctx: PatternContext) -> KnownPattern:
        ...
        if is_pow2:
            ...
            # with the 32-bit modular inverse, and the simplifier narrows further
            # still when only the low bits are read: `size() & 1` arrives as
            # `Mul(Conv(64->8, diff >> k), inv8)`. The narrowed product is the
            # narrowed size (the inverse survives reduction mod 2**w), so every
            # width's inverse is accepted. Accepting only the word-width magic was
...
#: Every opaque element size, power-of-two and not. The list is wide on purpose:
#: sizeof(T) is whatever the program's record happens to be (112 for
#: CryptoPP::ECPPoint, say), and a size that is not covered leaves the raw magic
#: multiply in the output, which is the shape this pattern exists to remove.
