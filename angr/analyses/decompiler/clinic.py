...
class Clinic(Analysis, Serializable):
    ...
    def _remove_call_used_x87_clearing(block, converted: ailment.Block) -> None:
        ...
        x87_regs_offset = block.arch.registers["fpu_regs"][0]
        x87_tags_offset = block.arch.registers["fpu_tags"][0]
        put_x87_reg = rf"PutI\({x87_regs_offset}:F64x8\)\[t\d+,0\] = t\d+"
        mark_x87_reg_live = rf"PutI\({x87_tags_offset}:I8x8\)\[t\d+,0\] = 0x01"
        mark_x87_reg_dead = rf"PutI\({x87_tags_offset}:I8x8\)\[t\d+,0\] = 0x00"
