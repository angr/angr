...
class Clinic(Analysis, Serializable):
    ...
    def downsize(self) -> None:
        """
        Drop all state that the decompiler's cache-reuse path does not consume. After downsizing, the Clinic
        supports exactly what ``Decompiler._decompile()`` needs to reuse it on a cache hit (cc_graph,
        unoptimized_graph, arg_vvars, arg_list, externs, and the primitive configuration); everything else is
        regenerated by a fresh decompilation run.
        """
        # regenerated fresh from an AIL graph whenever needed (populate_model)
        self.reaching_definitions = None
        # graph is restored from cc_graph by the reuse path before it is ever read
        self.graph = None
        self._ail_graph = None
        self._init_ail_graph = None
        self._init_arg_vvars = None
        self._type_hints = []
        self._blocks_by_addr_and_size = None
        self.func_args = None
        self.func_ret_var = None
        self.data_refs = {}
        self._inline_functions = set()
        self._stackarg_offset_manager = StackArgOffsetManager(self.project.arch.bits if self.project else 64)
        # notes persist on the codegen and the decompilation cache; unlink the shared dict without mutating it
        self.notes = {}
        # runtime heavies that were never serialized
        self.typehoon = None
        self._spt = None
        self.optimization_scratch = {}
        self._cache = None
    ...
    def parse_from_cmessage(
        ...
        **kwargs,
        ...
        are indistinguishable. If ``function`` is None and ``kb`` is provided, the function is resolved by address
        from the cmessage."""
        msg = cmsg
