    ...
    def _rewrite_indirect_register_accesses(self, ail_graph):
        ...
        def try_resolve_ireg(ireg: AILIReg) -> AILReg | None:
            ...
            if offset is not None:
                return AILReg(ireg.idx, offset, ireg.bits, **ireg.tags)
            ...
            if isinstance(ireg.reg_offset, AILTmp):
                ...
                if repl is not None and repl.concrete:
                    return AILReg(ireg.idx, _compute_offset(ireg, repl.concrete_value), ireg.bits, **ireg.tags)
            ...
            for seed_offset, _, _ in index_reg_seeds:
                ...
                if val is not None:
                    return AILReg(ireg.idx, _compute_offset(ireg, val), ireg.bits, **ireg.tags)
