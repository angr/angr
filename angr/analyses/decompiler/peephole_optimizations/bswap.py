    ...
    def optimize(self, expr: BinaryOp, **kwargs):
        ...
        if expr.op == "Or":
            ...
            if len(or_pieces) == 4:
                # parse pieces
                shifts = set()
                # NOTE: collected as a list and compared with .likes(). AIL __eq__ is
                # idx-sensitive, so in a set the four textually identical cores of a
                # real byte swap, each a separate occurrence with its own index, land
                # as four distinct members and the match is abandoned. The hand-written
                # fixture happens to reuse one object four times, which is why this went
                # unnoticed; on real code it made the 32-bit rewrite fire on 12 of 511
