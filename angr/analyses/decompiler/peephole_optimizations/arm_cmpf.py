...
class ARMCmpF(PeepholeOptimizationExprBase):
    ...
    def optimize(self, expr: Convert):
        ...
        if expr.from_bits == 32 and expr.to_bits == 1:
            ...
            negate = False
            bit_mask = None
            if isinstance(convert_from, BinaryOp) and convert_from.op == "Xor" \
                    and isinstance(convert_from.operands[1], Const) and convert_from.operands[1].value == 1:
                negate = True
                convert_from = convert_from.operands[0]
            if isinstance(convert_from, BinaryOp) and convert_from.op == "And" \
    ...
    def _match_nzcv_bits_extraction(expr: BinaryOp):
        ...
        if expr.op == "Or" and isinstance(expr.operands[1], BinaryOp) and expr.operands[1].op == "Xor":
            ...
            if isinstance(chunk0, BinaryOp) and chunk0.op == "Shr" \
                    and isinstance(chunk0.operands[1], Const) and chunk0.operands[1].value == 0x1e:
                if isinstance(chunk1, BinaryOp) and chunk1.op == "Shr" \
                        and isinstance(chunk1.operands[1], Const) and chunk1.operands[1].value == 0x1f:
                    if isinstance(chunk2, BinaryOp) and chunk2.op == "Shr" \
                            and isinstance(chunk2.operands[1], Const) and chunk2.operands[1].value == 0x1c:
    ...
    def _match_nzcv(expr: BinaryOp):
        ...
        if expr.op == "And" and isinstance(expr.operands[1], Const) and expr.operands[1].value == 0xf000_0000:
            ...
            if isinstance(inner, BinaryOp) and inner.op == "Shl" \
                    and isinstance(inner.operands[1], Const) and inner.operands[1].value == 0x1c:
    ...
    def _match_termL(expr: BinaryOp):
        ...
        if isinstance(expr, BinaryOp) and expr.op == "Add" \
                and isinstance(expr.operands[1], Const) and expr.operands[1].value == 1:
            lhs0 = expr.operands[0]
            if isinstance(lhs0, BinaryOp) and lhs0.op == "Shr" \
                    and isinstance(lhs0.operands[1], Const) and lhs0.operands[1].value == 0x1d:
                lhs1 = lhs0.operands[0]
                if isinstance(lhs1, BinaryOp) and lhs1.op == "Sub" \
                        and isinstance(lhs1.operands[1], Const) and lhs1.operands[1].value == 1:
                    lhs2 = lhs1.operands[0]
                    if isinstance(lhs2, BinaryOp) and lhs2.op == "Shl" \
                            and isinstance(lhs2.operands[1], Const) and lhs2.operands[1].value == 0x1e:
                        lhs3 = lhs2.operands[0]
                        if isinstance(lhs3, BinaryOp) and lhs3.op == "Xor" \
                                and isinstance(lhs3.operands[1], Const) and lhs3.operands[1].value == 1:
                            ix = lhs3.operands[0]
                            return True, ix
        return False, None
    ...
    def _match_termR(expr: BinaryOp):
        ...
        if isinstance(expr, BinaryOp) and expr.op == "And" \
            ...
            if isinstance(lhs0, BinaryOp) and lhs0.op == "And":
                ...
                if isinstance(op1, BinaryOp) and op1.op == "Shr" \
                        and isinstance(op1.operands[1], Const) and op1.operands[1].value == 1:
                    ix1 = op1.operands[0]
                    if ix0 == ix1:
                        return True, ix0
        return False, None
    ...
    def _match_ix(expr: BinaryOp):
        ...
        if isinstance(expr, BinaryOp) and expr.op == "Or":
            ...
            if isinstance(left, BinaryOp) and left.op == "And" \
                    and isinstance(left.operands[1], Const) and left.operands[1].value == 3:
                left_inner = left.operands[0]
                if isinstance(left_inner, BinaryOp) and left_inner.op == "Shr" \
                        and isinstance(left_inner.operands[1], Const) and left_inner.operands[1].value == 5:
                    cmpf_0 = left_inner.operands[0]
            if isinstance(right, BinaryOp) and right.op == "And" \
                    and isinstance(right.operands[1], Const) and right.operands[1].value == 1:
