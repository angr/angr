            ...
            if isinstance(cond_n_op, Expr.Const) and isinstance(cond_n_op, Expr.Const):
                ...
                if cond_v in { ARMCondHS, ARMCondLO }:
                    ...
                    if op_v == ARMG_CC_OP_SBB:
                        ...
                        if dep_3.value == 0:
                            r = Expr.BinaryOp(ccall.idx, "CmpGE" if inv == 0 else "CmpLT",
                                              (dep_1, dep_2),
                                              False,
                                              **ccall.tags)
                        else:
                            r = Expr.BinaryOp(ccall.idx, "CmpGT" if inv == 0 else "CmpLE",
                                              (dep_1, dep_2),
                                              False,
                                              **ccall.tags)
                    ...
                    if op_v == ARMG_CC_OP_SUB:
                        # dep_1 < dep_2,
                        #   and then negate the result if inv == 1
                        r = Expr.BinaryOp(ccall.idx, "CmpLT" if inv == 0 else "CmpGE",
                                          (dep_1, dep_2),
                                          False,
                                          **ccall.tags)
                        return Expr.Convert(None, r.bits, ccall.bits, False, r, **ccall.tags)
