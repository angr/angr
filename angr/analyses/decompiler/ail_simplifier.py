...
class AILSimplifier(Analysis):
    ...
    def _is_safe_replacement(stmt, src_expr, dst_expr):
        # Assume all parameters are registers
        if isinstance(dst_expr, VirtualVariable) and (dst_expr.was_reg or dst_expr.was_parameter):
    ...
    def _replace_expr_and_update_block(block, stmt_idx, stmt, src_expr, dst_expr) -> tuple[bool, Block | None]:
        # Special case: Do not generate Reference(reg vvar)
        # Check conservatively: If dst_expr is reg_vvar and stmt contains Reference(src_expr), skip
