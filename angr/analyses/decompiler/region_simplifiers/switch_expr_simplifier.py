...
class SwitchExpressionSimplifier(SequenceWalker):
    ...
    def _handle_SwitchCase(self, node: SwitchCaseNode, **kwargs):
        ...
        while isinstance(switch_expr, ailment.Expr.Convert):
            optimized = RemoveNoopConversions.optimize(switch_expr)
        ...
        if isinstance(switch_expr, ailment.Expr.BinaryOp):
            if switch_expr.op in {"Add", "Sub"} and isinstance(switch_expr.operands[1], ailment.Expr.Const):
