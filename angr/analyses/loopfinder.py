...
class LoopFinder(Analysis):
    ...
    def _parse_loop_graph(self, subg, bigg):
        ...
        for subloop in tops:
            if subloop.entry in loop_body_nodes:
                ...
                _subgraphs = (networkx.induced_subgraph(subg, nodes) for nodes in
    ...
    def _parse_loops_from_graph(self, graph):
        ...
        outtop = []
        outall = []
        for subg in ( networkx.induced_subgraph(graph, nodes) for nodes in networkx.strongly_connected_components(graph)):
            if len(subg.nodes()) == 1:
