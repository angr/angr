...
class FunctionManager[K: (int, SootMethodDescriptor)](KnowledgeBasePlugin, collections.abc.Mapping[K, Function]):
    ...
    def callgraph(self, graph: networkx.MultiDiGraph) -> None:
        ...
        # NetworkX views have no mutation callback of their own, but share their source's edge dictionaries.
        # Materialize them before binding observers or detaching the previous graph.
        callgraph = (
            graph
            if isinstance(graph, ObservableMultiDiGraph)
            and not hasattr(graph, "_graph")
            and (graph is old_graph or not graph.has_mutation_callback)
    ...
    def rebuild_callgraph(self):
        ...
        for func in self._function_map.values():
            owner = func._function_manager
            try:
                owns_functions &= owner is not None and owner._function_map is self._function_map
        ...
        # A shallow manager copy shares Functions whose mutations notify the source manager. Rebuilding produces
        # a current snapshot, but cannot certify that this manager will observe subsequent changes.
        self._function_graph_state.callgraph_trusted = owns_functions
