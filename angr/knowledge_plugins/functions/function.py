...
class Function(Serializable):
    ...
    def _set_transition_graph(self, graph: networkx.DiGraph, *, notify: bool) -> None:
        ...
        if (
            hasattr(graph, "_graph")
            or (isinstance(graph, ObservableDiGraph) and graph is not old_graph and graph.has_mutation_callback)
            or any(node_is_bound_elsewhere(node) for node in graph)
        ):
