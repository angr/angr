...
class _ObservableGraphMixin(networkx.DiGraph if TYPE_CHECKING else object):
    ...
    def _validate_edges_from(
        self, edges: Iterable, candidates: list, edge_states: list, edge_attr_states: dict, binding_state
    ):
        for edge in edges:
            try:
            ...
            if state is not None:
                edge_attr_states.setdefault(id(state[3]), state)
            yield edge

    def _track_removed_nodes(self, nodes: Iterable, candidates: list, edge_attr_states: dict):
        for node in nodes:
            try:
    ...
    def _track_removed_edges(self, edges: Iterable, edge_states: list, edge_attr_states: dict):
        for edge in edges:
            try:
                source, destination = edge[0], edge[1]
            except (IndexError, TypeError):
    ...
    def _capture_removable_edge_attr(self, source, destination, key, edge_attr_states: dict) -> None:
        ...
        state = self._edge_attr_state(source, destination, key)
        if state is not None:
            # Retain the first snapshot of each dictionary without rescanning all previously captured edges.
            edge_attr_states.setdefault(id(state[3]), state)

    def _capture_incident_edge_attrs(self, node, edge_attr_states: dict) -> None:
        if self.is_multigraph():
    ...
    def _all_edge_attr_states(self) -> dict:
        states = {}
        if self.is_multigraph():
    ...
    def remove_node(self, n):
        ...
        try:
            ...
            self._detach_removed_edge_attrs(edge_attr_states.values())
    ...
    def add_edges_from(self, ebunch_to_add: Iterable, **attr):
        ...
        return self._run_topology_mutation(
            ...
            lambda: self._topology_was_added(candidates, edge_states, edge_attr_states.values()),
        )
    ...
    def clear(self):
        ...
        try:
            ...
            self._detach_removed_edge_attrs(edge_attr_data.values())
