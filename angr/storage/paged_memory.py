...
class SimPagedMemory(object):
        ...
        for c in candidates:
            if c not in self and c in other:
                ...
                if type(self[c]) is not SimMemoryObject:
                    self[c] = SimMemoryObject(self.state.se.BVV(self[c], archinfo.BYTE_BITS), c)
                if type(other[c]) is not SimMemoryObject:
                    other[c] = SimMemoryObject(self.state.se.BVV(other[c], archinfo.BYTE_BITS), c)
    ...
    def map_region(self, addr, length, permissions, init_zero=False):
        ...
        for page in xrange(pages):
            ...
            if init_zero:
                mo = SimMemoryObject(claripy.BVV(0, self._page_size * archinfo.BYTE_BITS), page_id*self._page_size)
