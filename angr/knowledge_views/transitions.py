...
class TransitionsView(KnowledgeBaseView):
    ...
    def reconstruct(self):
        ...
        for block in BlockView(self._kb).iter_blocks():
            from_addr = block.start

            direct_jumps = self._collect_exits(block)
            ...
            indirect_jumps = self._kb.indirect_jumps[from_addr]
            for to_addr, specs in indirect_jumps.iteritems():
                jumpkind, insn_addr, stmt_idx = specs
                self._add_transition(from_addr, to_addr, jumpkind, insn_addr, stmt_idx)

            processed_set.add(from_addr)

        for from_addr in set(self._kb.indirect_jumps) - processed_set:
            indirect_jumps = self._kb.indirect_jumps[from_addr]
            for to_addr, specs in indirect_jumps.iteritems():
                jumpkind, insn_addr, stmt_idx = specs
                self._add_transition(from_addr, to_addr, jumpkind, insn_addr, stmt_idx)
