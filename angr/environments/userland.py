...
from ..sim_environment import IRange, SimEnvironment
...
l = logging.getLogger("angr.environments.sim_userland")

class SimUserland(SimEnvironment):
    """
    ...
    def state_blank(self, **kwargs):
        ...
        state = super(SimUserland, self).state_blank(**kwargs)
        return state
...
class SimWindows(SimEnvironment):
    """
        ...
        for arg in args:
            ...
            if type(arg) is str:
                if '"' in arg or '\0' in arg:
                    raise AngrSimEnvironmentError("Can't handle windows args with quotes or nulls in them")
                arg = claripy.BVV(arg)
...
os_mapping = defaultdict(lambda: SimEnvironment)
