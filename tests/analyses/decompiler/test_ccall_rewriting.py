...
class TestAMD64CCallRewriterCondZShl(unittest.TestCase):
    """CondZ/CondNZ over SHL: ZF = (shift result == 0), narrowed to the op width like the SHR arm."""

    def test_condz_shl_narrows_to_op_width(self):
        for op, bits in (
            ("G_CC_OP_SHLB", 8),
            ("G_CC_OP_SHLW", 16),
            ("G_CC_OP_SHLL", 32),
            ...
            for cond, expr_op in (("CondZ", "CmpEQ"), ("CondNZ", "CmpNE")):
                cmp = _unwrap_convert(_rewrite(_make_ccall(AMD64_CondTypes[cond], AMD64_OpTypes[op])))
                assert isinstance(cmp, Expr.BinaryOp), f"{cond} x {op}: not rewritten"
                assert cmp.op == expr_op, f"{cond} x {op}: got {cmp.op}"
                assert cmp.operands[0].bits == bits, f"{cond} x {op}: operand is {cmp.operands[0].bits}-bit"
                assert cmp.operands[1].value_int == 0, f"{cond} x {op}: expected comparison against 0"

    def test_condz_shlb_ignores_upper_bits(self):
        # low byte is zero (ZF set) while bits above it are non-zero
        cmp = _unwrap_convert(
            _rewrite(_make_ccall(AMD64_CondTypes["CondZ"], AMD64_OpTypes["G_CC_OP_SHLB"], Expr.Const(1, 0x100, 64)))
        )
        assert cmp.op == "CmpEQ"
        lhs, rhs = cmp.operands
        assert lhs.value_int == 0 and lhs.bits == 8
...
class TestAMD64CCallRewriterDifferential(unittest.TestCase):
    ...
    def test_condz_shlb_differential(self):
        self._sweep("CondZ", "G_CC_OP_SHLB")

    def test_condnz_shlb_differential(self):
        self._sweep("CondNZ", "G_CC_OP_SHLB")
