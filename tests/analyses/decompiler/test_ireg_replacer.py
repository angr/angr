...
class TestIRegReplacerEngine(unittest.TestCase):
    ...
    def test_resolve_const_ireg_in_assignment_dst(self):
        ...
        ix = Const(1, 0xFFFFFFFF, 32)
        ireg = IRegister(2, ix, 64, array_base=904, array_nElems=8, array_shift=3)
        data = Const(3, 42, 64)
    ...
    def test_resolve_const_ireg_in_expression(self):
        ...
        ix = Const(1, 0, 32)  # ftop=0 -> offset 904
        ireg = IRegister(2, ix, 64, array_base=904, array_nElems=8, array_shift=3)
        dst = Register(3, 16, 64)  # rax
    ...
    def test_unresolvable_ireg_assignment_stripped(self):
        ...
        tmp = Tmp(1, 5, 32)
        ireg = IRegister(2, tmp, 8, array_base=968, array_nElems=8, array_shift=0)
        data = Const(3, 0, 8)
        stmt = Assignment(0, ireg, data, ins_addr=0x1000)

        # Also add a normal statement to ensure it survives
        normal = Assignment(1, Register(4, 16, 64), Const(5, 99, 64), ins_addr=0x1000)
    ...
    def test_fptag_resolution(self):
        ...
        ix = Const(1, 3, 32)  # ftop=3
        ireg = IRegister(2, ix, 8, array_base=968, array_nElems=8, array_shift=0)
        data = Const(3, 1, 8)
        stmt = Assignment(0, ireg, data, ins_addr=0x1000)
