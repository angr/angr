...
class TestLinkedListPatterns(TestCase):
    # each idiom is exercised on BOTH the WDK PE and the Linux ELF, showing that one
...
class TestVectorExactDivSize(TestCase):
    ...
    # followed by a plain multiply by the modular inverse of the odd part rather
    # than the usual mulhi-and-shift. Both operands follow from sizeof(T), so the
...
class TestPatternsOutlineDuringDecompilation(TestCase):
    ...
    # for the final (post-de-phi) form, the bug that made c_str match after
    # decompilation but not during it. (std::swap: see test_known_patterns.TestAutomaticPipeline;
...
class TestBswapPeephole(TestCase):
    # This is the bswap peephole, not a KnownPattern, but it is calibrated on the
    # same fixture. gcc recognizes the hand-rolled shift/mask spellings too, so the
...
class TestKernelTargetGate(TestCase):
    ...
    # IoCsqInitialize, an unmistakable true positive.
...
class TestCorroborationGate(TestCase):
    ...
    def test_uncorroborated_front_stays_raw(self):
        # str_front(const std::string &s) { return s.front(); } has nothing else in
        # the function that says "string", so the gate stays shut
        proj = angr.Project(STL2_BIN, auto_load_libs=False)
        cfg = proj.analyses.CFGFast(normalize=True)
        proj.analyses.CompleteCallingConventions(cfg=cfg.model)
