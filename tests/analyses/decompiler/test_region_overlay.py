...
class TestActiveMode(unittest.TestCase):
    def test_active_views_update_incrementally(self):
        g, n = diamond()
        mgr = OverlayManager(g)
        mgr.root.head = n[1]
        sub = mgr.root.create_subregion(n[2], [n[2], n[3], n[4], n[5]], cyclic=False)

        sub.begin_structuring()
        assert sub.active
        active_graph = sub.view()
        active_gws = sub.view_with_successors()
        assert sub.successor_nodes() == {n[6]}

        seq = Node(0x20)
        sub.replace_nodes(n[2], seq, old_node_1=n[3])
        # the same working graph objects are updated in place
        assert sub.view() is active_graph
        assert sub.view_with_successors() is active_gws
        assert seq in active_graph and n[2] not in active_graph and n[3] not in active_graph
        assert edge_set(active_graph) == {(seq, n[4]), (seq, n[5]), (n[4], n[5])}
        assert (n[5], n[6]) in edge_set(active_gws)
        # external in-edges do not leak into the member view
        assert n[1] not in active_graph
        # ... but the shared graph is rewired
        assert g.has_edge(n[1], seq)

        sub.hide_edge(n[4], n[5])
        assert (n[4], n[5]) not in edge_set(active_graph)
        assert (n[4], n[5]) not in edge_set(active_gws)
        assert g.has_edge(n[4], n[5])

        sub.remove_node(n[6])  # successor removal
        assert n[6] not in active_gws
        assert sub.successor_nodes() == set()
    ...
    def test_active_structure_and_finalize_then_rollback(self):
        g, n = diamond()
        mgr = OverlayManager(g)
        mgr.root.head = n[1]
        sub = mgr.root.create_subregion(n[2], [n[2], n[3], n[4], n[5]], cyclic=False)
        ...
        before_edges = set(g.edges())
        before_root_view = edge_set(mgr.root.view())

        chk = mgr.checkpoint()
        sub.begin_structuring()
        m1 = Node(0x21)
        sub.replace_nodes(n[2], m1, old_node_1=n[3])
        m2 = Node(0x22)
        sub.replace_nodes(m1, m2, old_node_1=n[4])
        m3 = Node(0x23)
        sub.replace_nodes(m2, m3, old_node_1=n[5])
        result = sub.finalize(m3)
        assert not sub.active
        assert result is m3
        assert edge_set(mgr.root.view()) == {(n[1], m3), (m3, n[6])}

        mgr.rollback(chk)
        assert set(g.edges()) == before_edges
        assert edge_set(mgr.root.view()) == before_root_view
        assert sub in mgr.root.children
        assert set(sub.members) == {n[2], n[3], n[4], n[5]}
        assert not sub.active

    def test_active_dissolve(self):
        g, n = diamond()
        mgr = OverlayManager(g)
        mgr.root.head = n[1]
        sub = mgr.root.create_subregion(n[2], [n[2], n[3], n[4], n[5]], cyclic=False)
