...
class TestSplitReturnValues(unittest.TestCase):
    ...
    def test_a_user_rename_survives_redecompilation(self):
        # the second Clinic pass drops kb.dec_variables so it can number its locals from scratch, and that is
        # where user renames live: they are snapshotted and restored around it
        bin_path = os.path.join(test_location, "i386", "nl")
        proj = angr.Project(bin_path, auto_load_libs=False)
        cfg = proj.analyses.CFGFast(normalize=True, data_references=True)
        func = cfg.functions["__udivdi3"]

        dec = proj.analyses[Decompiler].prep(fail_fast=True)(func, cfg=cfg.model)
        assert dec.codegen is not None and dec.codegen.text is not None
        names = DECL_RE.findall(dec.codegen.text)
        assert names
        rename_variable(proj, func, names[0], "picked_by_user")

        dec = proj.analyses[Decompiler].prep(fail_fast=True)(func, cfg=cfg.model, regen_clinic=True)
        assert dec.codegen is not None and dec.codegen.text is not None
        print_decompilation_result(dec)
        names = DECL_RE.findall(dec.codegen.text)
