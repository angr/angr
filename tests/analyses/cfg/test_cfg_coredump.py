...
class TestCfgCoredump(unittest.TestCase):
    def test_core_mappings_are_not_all_code(self):
        ...
        # Scoping the analysis to that one blob answers for that blob. An empty map from an object
        # nothing claims to have read falls back to every backer in the loader, so the answer would
        # otherwise be the whole dump.
        scoped = project.analyses.CFGFast(binary=target, force_complete_scan=False, normalize=False)
        assert scoped._exec_mem_regions == []
