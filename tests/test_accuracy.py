...
def emulate(arch, binary, use_sim_procs, steps, hit_addrs, finished):
    p = angr.Project(os.path.join(test_location, arch, binary), use_sim_procedures=use_sim_procs, rebase_granularity=0x1000000, load_debug_info=False, auto_load_libs=True)# auto_load_libs can't be disabled as the test takes longer time to execute
...
def test_locale():
    p = angr.Project(os.path.join(test_location, 'i386', 'isalnum'), use_sim_procedures=False, auto_load_libs=True)# auto_load_libs can't be disabled as the test takes longer time to execute
