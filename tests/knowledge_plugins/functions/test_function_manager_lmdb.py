...
class TestFunctionManagerLMDB(unittest.TestCase):
    ...
    def test_spilling_dict_copy_grows_a_full_map(self):
        """
        copy() writes the spilled records into a new sub-database. When that write fills the LMDB map it has to
        grow the map and retry; before, it had no MapFullError handling and the copy failed outright.
        """
        proj, function_map = self._spilled_function_map()
        rtdb = proj.kb.rtdb
        addrs = set(function_map)
        mapsize_before = rtdb._lmdb_mapsize

        # fill the map without growing it, so the next write into it has nowhere to go
        junk = rtdb.open_db("junk")
        records = 0
        for size in (1 << 20, 1 << 16, 1 << 12, 1 << 9):
            while True:
                try:
                    with rtdb.begin_txn(junk, write=True) as txn:
                        txn.put(str(records).encode("utf-8"), os.urandom(size))
                except lmdb.MapFullError:
                    break
                records += 1
        assert records, "the map never filled up"

        new_map = function_map.copy()

        assert rtdb._lmdb_mapsize > mapsize_before, "copy() did not grow the map"
