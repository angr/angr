...
from angr.sim_type import SimStruct, SimType, SimTypeFunction, SimTypeInt, SimTypePointer, SimTypeRef, SimTypeShort
...
class TestFunctionParserPrototype(unittest.TestCase):
    ...
    def test_struct_that_only_shares_its_name_with_a_definition_is_stored_in_full(self):
        # libc's `struct netent` and winsock's differ in n_addrtype, int against short. libc declares no type
        # collection, so its prototypes are resolved against every collection the process has loaded, and once
        # anything has loaded the win32 definitions a reference to `netent` restores winsock's.
        angr.procedures.definitions.load_win32api_definitions()
        proj = angr.Project(os.path.join(test_location, "x86_64", "fauxware"), auto_load_libs=False)
        func = self._function_with_prototype(proj, "libc.so.6", "getnetbyname")

        before = func.prototype
        assert isinstance(before, SimTypeFunction)
        assert isinstance(before.returnty, SimTypePointer)
        netent = before.returnty.pts_to
        assert isinstance(netent, SimStruct)
        assert netent.name == "netent"
        # SimTypeShort subclasses SimTypeInt, so these have to be exact types to tell the two apart
        assert type(netent.fields["n_addrtype"]) is SimTypeInt

        # the collection's definition of that name is a different struct
        defined = SIM_TYPE_COLLECTIONS["win32"].get("netent")
        assert isinstance(defined, SimStruct)
        assert type(defined.fields["n_addrtype"]) is SimTypeShort

        stored, after = self._round_trip(proj, func)
        structs = []
        for prototype in (stored, after):
            returnty = prototype.returnty
            assert isinstance(returnty, SimTypePointer)
            struct = returnty.pts_to
            assert isinstance(struct, SimStruct)
            assert type(struct.fields["n_addrtype"]) is SimTypeInt
            structs.append(struct)
        # what comes back is libc's struct, not the collection's
        assert structs[-1] == netent
