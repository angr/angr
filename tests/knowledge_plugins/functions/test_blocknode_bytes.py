...
"""
BlockNode bytes: manual nodes store their bytes, every other node reads them from the loader at addr + delta.
"""
...
class TestBlockNodeBytes(unittest.TestCase):
    def test_thumb_delta(self):
        ...
        for n in thumb_nodes:
            ...
            assert n.bytestr == memory.load(n.addr - 1, n.size)
        ...
        for n in reloaded.transition_graph:
            if isinstance(n, BlockNode) and n.thumb:
                assert n.delta == -1 and n.bytestr == memory.load(n.addr - 1, n.size)
        # angrdb round trip
        ...
        assert all(n.delta == -1 and n.bytestr == proj2.loader.memory.load(n.addr - 1, n.size) for n in nodes2)
        # pickle
        n = thumb_nodes[0]
        p = pickle.loads(pickle.dumps(n))
        assert p == n and p.delta == -1 and p.thumb and not p.manual

    def test_manual_nodes(self):
        proj = angr.Project(FAUXWARE, auto_load_libs=False, cache_limits={"functions": 2})
        proj.analyses.CFGFast()
        with self.assertRaises(TypeError):
            BlockNode(0x500000, 4, bytestr=b"\x90" * 4)
        with self.assertRaises(TypeError):
            BlockNode(0x500000, 4).bytestr = b"\x90" * 4
        unmapped = BlockNode(0x500004, 4)
        assert unmapped.bytestr is None

        fm = proj.kb.functions
        func = fm.function(addr=0x500000, create=True)
        assert func is not None
        manual = BlockNode(0x500000, 4, bytestr=b"\x90" * 4, manual=True)
        fm._add_node(0x500000, manual)
        fm._add_transition_to(0x500000, manual, unmapped, ins_addr=0x500003, stmt_idx=-2)
        assert unmapped.owner is func and unmapped.bytestr is None
        assert func._graph.node_bytes(0) == b"\x90" * 4
        ...
        assert node.manual and node.bytestr == b"\x90" * 4 and node.delta == 0
        assert loaded.code_nodes[0x500004].bytestr is None and not loaded.code_nodes[0x500004].manual
        assert {(n.addr, n.manual) for n in loaded.transition_graph} == {(0x500000, True), (0x500004, False)}
        assert [b.bytes for b in loaded.blocks if b.addr == 0x500000] == [b"\x90" * 4]

        # pickle and angrdb
        p = pickle.loads(pickle.dumps(loaded))
        assert p.code_nodes[0x500000].bytestr == b"\x90" * 4 and p.code_nodes[0x500000].manual
        assert pickle.loads(pickle.dumps(manual)).bytestr == b"\x90" * 4
        with tempfile.TemporaryDirectory() as td:
            path = os.path.join(td, "manual.adb")
            AngrDB(proj).dump(path)
            proj2 = AngrDB().load(path)
