...
from angr.knowledge_plugins.functions.observable_graph import ObservableDiGraph, ObservableMultiDiGraph, _ObservableDict
...
class TestFunctionManager(unittest.TestCase):
    ...
    def test_funcnode_copy_and_pickle_preserve_semantic_identity(self):
        ...
        with self.assertRaisesRegex(TypeError, "pickle state type"):
            mock.Mock(wraps=rejected.__setstate__)(None)
    ...
    def test_hooknode_copy_and_pickle_preserve_semantic_identity(self):
        for node_type in (HookNode, SyscallNode):
            with self.subTest(node_type=node_type.__name__):
                node = node_type(0xDEAD_1072, 7, None, thumb=True)
                for clone in (copy.copy(node), copy.deepcopy(node), pickle.loads(pickle.dumps(node, -1))):
                    assert type(clone) is type(node)
                    assert clone == node
                    assert hash(clone) == hash(node)
                    assert clone.thumb is True
                    assert clone._graph_owner() is None
                    assert not clone._semantic_identity_sealed

                legacy = node_type.__new__(node_type)
                legacy.__setstate__((node.addr, node.size, node.sim_procedure))
                assert legacy == node_type(node.addr, node.size, node.sim_procedure)
                assert legacy.thumb is False
                ...
                function.transition_graph.add_node(node)
                for clone in (function.copy(), pickle.loads(pickle.dumps(function, -1))):
                    copied_node = next(iter(clone.transition_graph))
                    assert copied_node is not node
                    assert copied_node == node
                    assert hash(copied_node) == hash(node)
                    assert copied_node.thumb is True
                    assert copied_node._graph_owner() is clone.transition_graph
                    assert copied_node._semantic_identity_sealed
    ...
    def test_function_graph_ownership_and_snapshot_views(self):
        ...
        self.enterContext(mock.patch.object(main, "tags", {"nested": ["source"]}))
        prototype = SimTypeFunction((SimTypePointer(SimTypeBottom()),), SimTypeInt()).with_arch(proj.arch)
        assert isinstance(prototype, SimTypeFunction)
        main.prototype = prototype
        ...
        copied_tags = getattr(copied, "tags", None)
        assert isinstance(copied_tags, dict)
        copied_tags["nested"].append("copy")
        assert main.tags == {"nested": ["source"]}
        assert isinstance(main.prototype, SimTypeFunction)
        assert isinstance(copied.prototype, SimTypeFunction)
        assert isinstance(copied_twice.prototype, SimTypeFunction)
        assert isinstance(main.calling_convention, SimCCUsercall)
        assert isinstance(copied.calling_convention, SimCCUsercall)
        assert isinstance(copied_twice.calling_convention, SimCCUsercall)
        ...
        assert copied.prototype.returnty is not None
        assert main.prototype.returnty is not None
        ...
        copied_pointer = copied.prototype.args[0]
        assert isinstance(copied_pointer, SimTypePointer)
        copied_pointer.offset = 4
        copied_pointer.pts_to.label = "copy-only"
        ...
        assert isinstance(copied.calling_convention.args[0], SimRegArg)
        assert isinstance(copied.calling_convention.ret_loc, SimRegArg)
        ...
        assert isinstance(main.prototype.args[0], SimTypePointer)
        ...
        assert isinstance(main.calling_convention.args[0], SimRegArg)
        assert isinstance(main.calling_convention.ret_loc, SimRegArg)
        ...
        assert isinstance(copied_twice.prototype.args[0], SimTypePointer)
        ...
        assert isinstance(copied_twice.calling_convention.args[0], SimRegArg)
        assert isinstance(copied_twice.calling_convention.ret_loc, SimRegArg)
    ...
    def test_function_manager_copy_and_replacement_ownership(self):
        ...
        assert main._function_manager is not None
        ...
        assert replacement._function_manager is not None
    ...
    def test_rebuilding_shallow_manager_copy_cannot_certify_callgraph(self):
        ...
        manager_copy = functions.copy()
        assert not manager_copy.callgraph_trusted
        manager_copy.rebuild_callgraph()
        assert not manager_copy.callgraph_trusted
        assert manager_copy[main.addr] is main
        source_generation = functions.function_graph_generation
        copy_generation = manager_copy.function_graph_generation
        _, callee, data = next(
            (src, dst, data) for src, dst, data in main.transition_graph.edges(data=True) if data.get("type") == "call"
        )
        assert manager_copy.callgraph.has_edge(main.addr, callee.addr)
        ...
        # A snapshot can become fully observed after every shared Function has been replaced with an owned copy.
        for addr, function in list(manager_copy.items()):
            manager_copy[addr] = function.copy()
        manager_copy.rebuild_callgraph()
        assert manager_copy.callgraph_trusted
        owned_main = manager_copy[main.addr]
        copy_generation = manager_copy.function_graph_generation
        source_generation = functions.function_graph_generation
        owned_main.transition_graph.add_node(BlockNode(0xDEAD_1073, 1))
        assert manager_copy.function_graph_generation != copy_generation
        assert functions.function_graph_generation == source_generation
        assert not manager_copy.callgraph_trusted

        temporary_manager = FunctionManager(proj.kb)
        temporary_manager[main.addr] = main.copy()
        temporary_manager[main.addr]._set_function_manager(weakref.proxy(temporary_manager))
        orphaned_copy = temporary_manager.copy()
        owner_ref = weakref.ref(temporary_manager)
        del temporary_manager
        gc.collect()
        assert owner_ref() is None
        orphaned_copy.rebuild_callgraph()
        assert not orphaned_copy.callgraph_trusted

    def test_graph_views_are_materialized_before_installation(self):
        def graph_view(graph, kind):
            if kind == "subgraph":
                return graph.subgraph(list(graph))
            if kind == "edge_subgraph":
                return graph.edge_subgraph(list(graph.edges))
            if kind == "reverse":
                return graph.reverse(copy=False)
            return graph.copy(as_view=True)

        for kind in ("subgraph", "edge_subgraph", "reverse", "copy"):
            for same_manager in (True, False):
                with self.subTest(kind=kind, same_manager=same_manager):
                    proj = angr.Project(os.path.join(test_location, "x86_64", "fauxware"), auto_load_libs=False)
                    proj.analyses.CFGFast(normalize=True)
                    source_manager = proj.kb.functions
                    target_manager = source_manager if same_manager else angr.KnowledgeBase(proj).functions
                    source_graph = source_manager.callgraph
                    view = graph_view(source_graph, kind)
                    src, dst, key, data = next(iter(view.edges(keys=True, data=True)))
                    target_manager.callgraph = view
                    installed_data = target_manager.callgraph[src][dst][key]
                    assert installed_data is not data
                    source_generation = source_manager.function_graph_generation
                    target_generation = target_manager.function_graph_generation
                    installed_data["type"] = "installed"
                    assert target_manager.function_graph_generation != target_generation
                    if not same_manager:
                        assert source_manager.function_graph_generation == source_generation
                    source_generation = source_manager.function_graph_generation
                    target_generation = target_manager.function_graph_generation
                    data["type"] = "source"
                    assert target_manager.function_graph_generation == target_generation
                    if not same_manager:
                        assert source_manager.function_graph_generation != source_generation
                    target_manager.callgraph.add_edge(src, dst, type="new")

                    source_function = source_manager["main"]
                    if same_manager:
                        target_function = source_function
                    else:
                        target_manager[source_function.addr] = source_function.copy()
                        target_function = target_manager[source_function.addr]
                    source_graph = source_function.transition_graph
                    view = graph_view(source_graph, kind)
                    src, dst, data = next(iter(view.edges(data=True)))
                    target_function.transition_graph = view
                    installed_data = target_function.transition_graph[src][dst]
                    assert installed_data is not data
                    source_generation = source_manager.function_graph_generation
                    target_generation = target_manager.function_graph_generation
                    installed_data["type"] = "installed"
                    ...
                    if not same_manager:
                        assert source_manager.function_graph_generation != source_generation
                    assert all(
                        node._graph_owner() is target_function.transition_graph
    ...
    def test_legacy_manager_pickle_rebuilds_key_and_argument_caches(self):
        ...
        function_map = mock.Mock(wraps=FunctionDict)(None, key_types=proj.kb.functions.function_address_types)
        assert isinstance(function_map, FunctionDict)
    ...
    def test_public_function_graph_mutations_advance_generation(self):
        ...
        assert outside_node not in target.graph
        ...
        assert outside_node in target.graph
    ...
    def test_bulk_graph_removals_detach_all_edge_handles(self):
        events = []
        ...
        for graph_type in (ObservableDiGraph, ObservableMultiDiGraph):
            for removal in ("clear", "clear_edges", "remove_nodes", "remove_edges"):
                with self.subTest(graph_type=graph_type.__name__, removal=removal):
                    graph = graph_type(mutation_callback=graph_mutated)
                    graph.add_edges_from((source, destination) for source in range(16) for destination in range(16))
                    if graph.is_multigraph():
                        graph.add_edges_from((source, source) for source in range(16))
                    handles = [data for _, _, data in graph.edges(data=True)]
                    events.clear()
                    if removal == "remove_nodes":
                        graph.remove_nodes_from([*graph, *graph])
                    elif removal == "remove_edges":
                        edges = list(graph.edges)
                        graph.remove_edges_from([*edges, *edges])
                    else:
                        getattr(graph, removal)()
                    assert len(events) == 1
    ...
    def test_observable_graph_adversarial_mutations_are_exact_and_atomic(self):
        ...
        def mutating_equality(side_key):
            value = mock.MagicMock()
            value.configure_mock(**{"__eq__.side_effect": lambda _other: MutatingTruth(side_key)})
            return value

        edge_data["value"] = mutating_equality("setitem_side_effect")
        graph_events.clear()
        edge_data["value"] = object()
        assert edge_data["setitem_side_effect"] is True
        assert len(graph_events) == 1

        edge_data["value"] = mutating_equality("update_side_effect")
        graph_events.clear()
        ...
        class GraphSnapshotEquality:
            def __init__(self):
                self.edge_data: dict[str, object] | None = None
    ...
    def test_failed_graph_replacements_preserve_installed_observers(self):
        ...
        with self.assertRaises((TypeError, NetworkXError)):
            mock.Mock(wraps=setattr)(main, "transition_graph", object())
        ...
        assert isinstance(replacement_edge_data, _ObservableDict)
        ...
        with self.assertRaises((TypeError, NetworkXError)):
            mock.Mock(wraps=setattr)(functions, "callgraph", object())
        ...
        assert isinstance(old_transition_edge_data, _ObservableDict)
