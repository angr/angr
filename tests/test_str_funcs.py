...
def test_strncpy():
    ...
    s = explorer.found[0]
    result = s.se.eval(s.memory.load(s.registers.load(16), 16), cast_to=bytes)
...
def test_strncpy_size():
    ...
    s = explorer.found[0]
    result = s.se.eval(s.memory.load(s.registers.load(16), 40), cast_to=bytes)
...
def test_strstr_and_strncpy():
    ...
    s = explorer.found[0]
    result = s.se.eval(s.memory.load(s.registers.load(16), 15), cast_to=bytes)
...
def test_strstr():
    ...
    s = explorer.found[0]
    result = s.se.eval(s.memory.load(s.registers.load(16), 9), cast_to=bytes)
